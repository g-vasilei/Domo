...
import { Eye, EyeOff, Check, Shield, Bell, BellOff, Thermometer } from 'lucide-react';
...
function PasswordInput({ value, onChange, placeholder }: { value: string; onChange: (v: string) => void; placeholder?: string }) {
  ...
  return (
    <div className="relative">
      ...
      />
      <button type="button" onClick={() => setVisible((v) => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400">
...
function SaveFeedback({ error, success }: { error: string; success: boolean }) {
  ...
  if (success) return <p className="text-sm text-emerald-500 flex items-center gap-1"><Check size={13} /> Saved</p>;
  return null;
}
...
function SectionHeader({ title, description }: { title: string; description?: string }) {
  return (
    <div>
      <h2 className="text-sm font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-widest">{title}</h2>
      ...
      <div className="flex flex-col gap-3">
        ...
        {isDenied ? (
          <p className="text-xs text-slate-400">
            Your browser has blocked notifications for this site. To enable them, click the lock icon in your browser's address bar and allow notifications.
          </p>
        ) : isGranted ? (
          <button
  ...
  return (
    <div className="flex flex-col gap-5 max-w-sm">
      ...
      <Field label="Confirm New Password">
        ...
        {confirm && next !== confirm && <p className="text-xs text-red-500">Passwords don't match</p>}
      </Field>
...
function TuyaTab() {
  ...
  const { data: me } = useQuery({ queryKey: ['me'], queryFn: () => api.get('/users/me').then((r) => r.data), staleTime: 60_000 });
  ...
  return (
    <div className="flex flex-col gap-5 max-w-sm">
      ...
      <Field label="Access Secret">
        <PasswordInput
          value={accessSecret}
          onChange={(v) => { setAccessSecret(v); setError(''); }}
...
function AlarmTab() {
  ...
  function close() { setStep(null); setCurrentPin(''); setNewPin(''); setError(''); }

  const mutation = useMutation({
    mutationFn: (confirm: string) =>
      api.post('/alarm/pin', { pin: confirm, ...(hasPinSet ? { currentPin } : {}) }),
    onSuccess: () => { close(); setSuccess(true); setTimeout(() => setSuccess(false), 3000); },
    onError: (e: any) => setError(e?.response?.data?.message ?? 'Failed to set PIN'),
  });

  function handleNew(pin: string) { setNewPin(pin); setError(''); setStep('confirm'); }
  function handleConfirm(pin: string) {
    if (pin !== newPin) { setError('PINs do not match'); setStep('new'); return; }
    mutation.mutate(pin);
  }
  ...
  return (
    <div className="flex flex-col gap-5 max-w-sm">
      <SectionHeader
        title="Alarm PIN"
        description={hasPinSet ? 'Required to arm and disarm the alarm.' : 'Create a PIN to protect arm and disarm actions.'}
      />
      ...
      {success && <p className="text-sm text-emerald-500 flex items-center gap-1"><Check size={13} /> PIN saved</p>}

      <button
        onClick={() => { setError(''); setStep(hasPinSet ? 'current' : 'new'); }}
        className="self-start px-5 py-2.5 bg-brand text-white rounded-md text-sm font-semibold hover:bg-brand/90 transition-colors"
      >
      ...
      {step === 'current' && (
        <PinPromptModal title="Current PIN" description="Enter your current PIN to continue"
          onConfirm={(pin) => { setCurrentPin(pin); setError(''); setStep('new'); }}
          onClose={close} error={error} />
      )}
      {step === 'new' && (
        <PinPromptModal title={hasPinSet ? 'New PIN' : 'Create PIN'} description="Enter your new PIN"
          onConfirm={handleNew} onClose={close} error={error} />
      )}
      {step === 'confirm' && (
        <PinPromptModal title="Confirm PIN" description="Re-enter your new PIN to confirm"
          onConfirm={handleConfirm} onClose={close} error={error} loading={mutation.isPending} />
      )}
    </div>
  );
}
...
function logBadge(log: any): { label: string; color: string } {
  ...
  if (ctx === 'alarm') {
    const map: Record<string, { label: string; color: string }> = {
      arm_home:  { label: 'Armed — Home', color: 'text-emerald-600 bg-emerald-50 dark:bg-emerald-500/10' },
    ...
    return map[action ?? ''] ?? { label: 'Alarm', color: 'text-slate-500 bg-slate-100 dark:bg-white/5' };
  }
  if (ctx === 'device') {
    const cmds: { code: string; value: unknown }[] = log.metadata?.commands ?? [];
    const summary = cmds.map((c) => `${c.code}: ${c.value}`).join(', ');
    return { label: summary || 'Device command', color: 'text-violet-600 bg-violet-50 dark:bg-violet-500/10' };
  }
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col gap-2">
        {[...Array(5)].map((_, i) => <div key={i} className="h-14 rounded-md bg-slate-100 dark:bg-white/5 animate-pulse" />)}
      </div>
    );
  }

  if (!logs.length) {
    return <div className="py-16 text-center text-slate-400"><p className="text-sm">No activity logged yet.</p></div>;
  }
  ...
  return (
    <div className="flex flex-col gap-1">
      {logs.map((log: any) => {
        ...
        return (
          <div key={log.id} className="flex items-center gap-3 px-3 py-3 rounded-md hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
            <span className={`text-xs font-medium px-2 py-0.5 rounded-full flex-shrink-0 whitespace-nowrap ${color}`}>{label}</span>
            <span className="text-sm text-slate-500 dark:text-slate-400 truncate flex-1">{log.user?.email ?? 'Unknown'}</span>
            <span className="text-xs text-slate-400 flex-shrink-0 whitespace-nowrap">{new Date(log.createdAt).toLocaleString()}</span>
          </div>
        );
      })}
    </div>
