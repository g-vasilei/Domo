...
import { Home, WifiOff, Cpu, GripVertical, ArrowLeftRight } from 'lucide-react';
import {
  DndContext,
  closestCenter,
  PointerSensor,
...
function applyOrder(rooms: any[], order: { rooms: string[]; devices: Record<string, string[]> }): any[] {
  let sorted = [...rooms];
...
// Sortable room wrapper
function SortableRoom({ room, rearranging, children }: { room: any; rearranging: boolean; children: React.ReactNode }) {
  ...
  return (
    <section>
      <div className="flex items-center gap-3 mb-3">
        ...
        <span className="text-xs text-slate-400">{online}/{room.devices.length} online</span>
      </div>
...
export default function DevicesPage() {
  const { data: rawRooms = [], isLoading, error } = useQuery({
    queryKey: ['rooms'],
  ...
  const activeDraggingDevice = activeId && !activeId.startsWith('room__')
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">All devices grouped by room</p>
        </div>
      ...
      {!isLoading && error && (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <h3 className="text-base font-semibold text-slate-700 dark:text-slate-300 mb-1">Could not load devices</h3>
      ...
      {!isLoading && !error && rooms.length === 0 && (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <h3 className="text-base font-semibold text-slate-700 dark:text-slate-300 mb-1">No devices found</h3>
          <p className="text-sm text-slate-400 max-w-xs">Add devices via the Smart Life app and assign them to rooms.</p>
        </div>
      )}
