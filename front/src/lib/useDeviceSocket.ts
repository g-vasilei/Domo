...
import { useAlarmStore, AlarmState } from '../store/alarm.store';
...
export function useDeviceSocket() {
  ...
  useEffect(() => {
    ...
    socket.on('device:update', ({ deviceId, commands }: { deviceId: string; commands: { code: string; value: unknown }[] }) => {
    ...
    socket.on('device:alert', (payload: {
      ...
      if (payload.type === 'battery_low') {
        ...
        useNotificationsStore.getState().push({ kind: 'battery', title, body, deviceId: payload.deviceId });
        fireBrowserNotif(title, body);
      } else if (payload.type === 'state_change') {
        const changes = (payload.commands ?? [])
          .map((c) => `${c.code}: ${c.value}`)
          .join(', ');
        const title = `📡 ${payload.deviceName} changed`;
        const body = changes || 'Device state updated';
        useNotificationsStore.getState().push({ kind: 'device', title, body, deviceId: payload.deviceId });
        fireBrowserNotif(title, body);
      }
    });
